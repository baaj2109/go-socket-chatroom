...
func handlerConnClose(conn net.Conn, connMap map[net.Conn]string, t int) {
	...
	if t == 2 {
		fmt.Printf("%s overtime ", connMap[conn])
		msg := "[" + time.Now().Format("2006-01-02 15:04:05") + "] - " + connMap[conn] + ": " + "overtime logout \n"
		_, err := conn.Write([]byte(msg))
		if err != nil {
			fmt.Printf("failed to send msg to  %s", connMap[conn])
...
func broadcast(conn net.Conn, connMap map[net.Conn]string, msg string) {
	for curConn := range connMap {
		msg := "[" + time.Now().Format("2006-01-02 15:04:05") + "] - " + connMap[conn] + ": " + msg + "\n"
		_, err := curConn.Write([]byte(msg))
		if err != nil {
